/** Códigos telefónicos internacionales (ITU). Orden: UY/AR primero, luego A–Z. */
...
export const COUNTRY_DIAL_CODES: readonly CountryDialCode[] = [
  ...
  { iso: 'EH', name: 'Sahara Occidental', dial: '+212' },
  ...
  { iso: 'SZ', name: 'Suazilandia', dial: '+268' },
...
const DIAL_CODES_BY_LENGTH = [...COUNTRY_DIAL_CODES]
  .map((c) => c.dial)
  .filter((dial, index, all) => all.indexOf(dial) === index)
  .sort((a, b) => b.length - a.length);

export function findCountryByIso(iso: string): CountryDialCode | undefined {
  return COUNTRY_DIAL_CODES.find((country) => country.iso === iso.toUpperCase());
}

/** Separa un teléfono guardado en ISO de país + número nacional. */
export function splitPhoneNumber(raw: string | undefined | null): {
  countryIso: string;
  nationalNumber: string;
} {
  const digitsOnly = (raw ?? '').replace(/[^\d+]/g, '');
  const normalized = digitsOnly.startsWith('+') ? digitsOnly : digitsOnly ? `+${digitsOnly}` : '';
  ...
  for (const dial of DIAL_CODES_BY_LENGTH) {
    if (normalized.startsWith(dial)) {
      const country = COUNTRY_DIAL_CODES.find((item) => item.dial === dial);
      return {
        countryIso: country?.iso ?? DEFAULT_COUNTRY_ISO,
        nationalNumber: normalized.slice(dial.length).replace(/\D/g, ''),
      };
    }
  }
...
export function composePhoneNumber(countryIso: string, nationalNumber: string): string {
  const country = findCountryByIso(countryIso);
  const dial = country?.dial ?? DEFAULT_DIAL_CODE;
  const local = nationalNumber.replace(/\D/g, '');
  return `${dial}${local}`;
}
